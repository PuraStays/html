...
app.service('sideBarSrvc', function($http, $localStorage){	
  ...
  var getWhen = function(){  
    if(angular.isUndefined($localStorage.when) || $localStorage.when === null){            
      return 0;     
    }else{
  ...
  var removeProgram = function(){
    programData = [];
    localStorage.setItem('puraprg', 0);
  }
...
app.service('bookingDataSrvc', function(sideBarSrvc){
  ...
  var allData = {
    ...
    "person": {"adult": sideBarSrvc.getPerson().adult.number, "child": sideBarSrvc.getPerson().child.number, "roomrequire": sideBarSrvc.getPerson().room},
    "rooms": roomdata,
  ...
  var getTotalRoomprice = function(){
  ...
  return{
    ...
    getTotalRoomprice : getTotalRoomprice 
  }  
});
